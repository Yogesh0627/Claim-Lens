...
import { CheckCircle2, MessageSquare, Send, UserPlus, Wand2, XCircle } from "lucide-react";
...
function AssignDialog({ claimId, onChanged }: { claimId: number; onChanged: () => void }) {
  ...
  const { data: investigators, error: listError } = useAsync(
    () => (open ? userService.list("INVESTIGATOR") : Promise.resolve([])),
    [open],
  );
...
function DecideDialog({ claimId, onChanged }: { claimId: number; onChanged: () => void }) {
  ...
  const decide = useMutation(
    (decision: "APPROVE" | "REJECT") =>
      claimService.decide(claimId, { decision, reason: reason || null }),
    {
